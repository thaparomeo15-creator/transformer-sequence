...
export default function WaitlistModal({ isOpen, onClose, tier }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (data.code === 'DUPLICATE') { setState('error'); setMessage('// SIGNAL ALREADY RECEIVED — YOU ARE IN THE QUEUE') }
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:'var(--color-bg-secondary)', border:'1px solid var(--glass-border)', backdropFilter:'var(--glass-blur)', borderRadius:'var(--radius-lg)', padding:'48px', maxWidth:'480px', width:'100%', position:'relative', zIndex:10 }}>
        ...
        {state === 'success' ? (
          <div style={{ textAlign:'center' }}>
            <div style={{ fontFamily:'var(--font-mono)', fontSize:'0.75rem', color:'var(--accent-cyan)', marginBottom:'24px' }}>// ACCESS QUEUED</div>
            ...
            <p style={{ color:'var(--color-text-secondary)', fontSize:'0.9rem', lineHeight:1.6 }}>Confirmation sent to {email}</p>
          </div>
        ) : (
          <>
            ...
            {state === 'error' && <div style={{ color:'#ff4444', fontSize:'0.6rem', fontFamily:'var(--font-mono)', marginBottom:'16px' }}>{message}</div>}
